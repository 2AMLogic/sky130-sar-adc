...
ROWS: tuple[Row, ...] = (
    ...
    Row(
        ...
        spec_anchor="spec/target-spec.md#target-table",
        conditions=(
            "Three of the four constituent mechanisms have now been probed "
            "individually; none of these campaigns is a sample-rate "
            "measurement. (a) CDAC-array bottom-plate-switch settling: "
            "`tt`/27C/1.8V single point only. (b) Comparator decision "
        ...
        verdict=(
            "UNMEASURED as an end-to-end sample-rate figure (three "
            "mechanisms probed individually, not combined); comparator "
            "decision delay is now a PVT-complete measurement, PASS on its "
            "own reset-integrity control (9/9 corners HELD)"
        ),
        notes=(
            "No end-to-end sample-rate campaign exists. Three of its input "
            "terms have been probed separately, and this row reports all "
            "three honestly rather than adding them up. (a) The CDAC array's "
            "own switch-R_on/top-plate settling is bounded at ONE corner: "
            "worst case 11.3861 ns (bit 8, rise), 7.3x inside the DR-006-"
            "derived 83.333 ns worst-case phase budget -- so the array's own "
            "settling is not the bottleneck at that corner. Switch R_on "
            "varies materially with process/temperature and no other corner "
            "has been run. (b) The comparator's own decision delay is now "
            "PVT-complete: issue #175 (DR-004 Amendment A) moved the "
            "cross-coupled NMOS latch pair's sources off hard-wired GND onto "
            "the input pair's own precharged drain nodes, closing the "
            "reset-integrity defect the prior pass surfaced (Vindiff = 0 mV "
            "control now HELD at all 9 ratified corners, 0.00 uA reset-phase "
            "static current at every corner). All 27/27 input-driven "
            "decision points resolved within the 15.0 ns evaluate window; "
            "binding corner `tt_27c_1.62v` at Vindiff = +0.5 mV, decision "
            "delay 4.3575 ns, 19.1x inside the DR-006-derived 83.333 ns "
            "worst-case phase budget (headroom against a DRAFT figure, not a "
            "pass against a ratified line). (c) The SAR sequencer's own "
            "CLK-to-phase-output logic delay is bounded at ONE corner across "
            "all 11 of its own ring-sequencer phase transitions: worst case "
            "0.3123 ns (phase b1), 266.9x inside the DR-006-derived 83.333 ns "
            "worst-case phase budget -- by far the smallest of the three "
            "mechanisms measured so far, and not the bottleneck at that "
            "corner. The sampling front end's own acquisition timing remains "
            "entirely unmeasured, so this is still not an end-to-end "
            "sample-rate number. Named as open work by spec/target-spec.md's "
            "own 'Not ratified by this record' list (#24/#28); DR-006's "
            "1.2-12 MHz clock range remains a mechanical consequence of this "
            "DRAFT row, not a derived result."
        ),
        sim_citations=(
