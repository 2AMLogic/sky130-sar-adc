...
ROWS: tuple[Row, ...] = (
    ...
    Row(
        ...
        conditions=(
            ...
            "once SAMPLE re-asserts): `tt`/27C/1.8V single point only, one "
            "step direction."
        ),
        verdict=(
            "UNMEASURED as an end-to-end sample-rate figure (four "
            "mechanisms probed individually, not combined); comparator "
            "decision delay is a PVT-complete measurement, PASS on its own "
            "reset-integrity control (9/9 corners HELD); three of the four "
            "mechanisms clear the DR-006 worst-case phase budget with a "
            "large margin, but the sampling front end's own acquisition "
            "does NOT clear it at this corner (see notes)"
        ),
        notes=(
            ...
            "CDAC array's own switch-R_on/top-plate settling is bounded at "
            "ONE corner: worst case 11.3861 ns (bit 8, rise), 7.3x inside "
            "the DR-006-derived 83.333 ns worst-case phase budget -- so the "
            "array's own settling is not the bottleneck at that corner. "
            "Switch R_on varies materially with process/temperature and no "
            "other corner has been run. (b) The comparator's own decision "
            ...
            "input value, once SAMPLE re-asserts, is bounded at ONE corner: "
            "the fast early settling (50%/90% of the way to the new value) "
            "is sub-ns on both nodes, but a slow secondary settling tail -- "
            "traced to the bootstrap precharge PFET `Sa`'s imperfect "
            "off-state once `BOOST_x` is driven above VDD, letting the "
            "boosted gate droop over tens of ns -- leaves a residual "
            "23.43 mV (single-ended, worst node) still uncorrected 83.333 ns "
            "after the acquiring edge, ~13.3x the provisional differential "
            "LSB's half-step. UNLIKE the other three mechanisms, this one "
            "does NOT clear the DR-006 worst-case (12 MHz) phase budget at "
            "this corner -- it is the first mechanism found to be a likely "
            "bottleneck rather than a comfortable margin, at the fast end "
            "of the DRAFT sample-rate range. Named as open work by "
            "spec/target-spec.md's own 'Not ratified by this record' list "
            "(#24/#28); DR-006's 1.2-12 MHz clock range remains a "
            "mechanical consequence of this DRAFT row, not a derived "
            "result, and DR-006's own deferred non-uniform-phase-allocation "
            "alternative may now be the more accurate model at the fast end."
        ),
        sim_citations=(
