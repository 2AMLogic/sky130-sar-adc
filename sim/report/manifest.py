...
ROWS: tuple[Row, ...] = (
    ...
    Row(
        ...
        conditions=(
            ...
            "(~4.43 pF/side more). The only two runs that measure THIS "
            "mechanism's own acquisition/hold residual on TOP_P/TOP_N at that "
            "combined load are tt-centric and pre-#236 -- the whole-ADC "
            "campaigns carry the same load while measuring other quantities; "
            "see (d) in the Notes and issue "
        ...
        notes=(
            ...
            "double. Exactly two runs in this tree measure THIS mechanism's "
            "own acquisition/hold residual on TOP_P/TOP_N at that combined "
            "load, both tt-centric and both pre-#236: "
            ...
            "measures that residual at the combined load. Assembled-load "
            "simulation itself is NOT absent, and this qualifier does not "
            "claim it is: `design/sar_adc_top.spice` ties the front end's "
            "TOP_P/TOP_N to the CDAC array's, so every campaign driving that "
            "netlist carries the same combined pair -- including "
            "`sim/full-conversion-transient/`'s full ratified 9-point "
            "post-#236 grid (whole-ADC code correctness: overall FAIL, 0/9 "
            "corners resolve every input to its ideal code +-1 LSB, "
            "root-caused to the open architecture/sizing decisions #267/#269 "
            "and not to acquisition) and `sim/supply-impedance-sensitivity/`'s "
            "supply-return networks on the same DUT. Neither measures the "
            "front end's own acquisition/hold residual on TOP_x, which is the "
            "one quantity this qualifier is about. Taking the combined-load "
            "DUT to the ratified grid is "
