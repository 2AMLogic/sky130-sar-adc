...
| Item 11 specifically (`klt erc` supply evidence) | **Cited and still `unmet` — but for a different, narrower reason than before** (issues **#344**, **#355**). The run of record is `layout/sar-adc-top/erc-reports/20260925-044420-f039594/` (`erc.json` + `record.md`), now cited by the manifest as a compound item-11 entry. Its **supply-continuity half passes**: `erc_status: clean`, **0** findings — all four declared supplies (`VDD`, `GND`, `VPWR`, `VGND`) at exactly one electrical island each, no `erc.supply_short`. That supersedes #344's first run (`20260923-143401-1ee4ba8/`, where `VPWR`/`VGND` each split into two std-cell islands that reached no top-level supply); #355 fixed it by tying both std-cell macros' met5 PDN straps together and out to two new top-level supply pins, per `spec/decision-records/DR-010-digital-supply-domain-partition.md`. The layout moved, not the gate, and that is checkable rather than asserted: the spec's *graded* fields (`stackup`, `vias`, `nets[]`, `ties_disclosure.kind` — all `klt erc` reads) canonicalise to the same digest `7f48fd89…` on every run from #344 to today, printed by `run-erc.sh` itself. Issue **#364** refreshed the spec's stale prose `_comment`s three times — once before issue #377's ground mesh landed, again after, and a third time to correct two GDS-specific shape counts a later Judge round found still described the superseded pre-mesh run — since none of the three moved the *layout* this spec is graded against, only the spec's prose, which moves the whole-file hash each time and nothing that digest covers. **The item is still `unmet`, and this row does not claim otherwise**: the grader's own reason is **`check_failed`** (both columns, `citation: null`) — `klt signoff`'s item-11 path tests the cited LVS part before the supply spec, and item 4's `mismatch` (klayout-tools#1878) stops it there, so the reason of record is *not* about the continuity half this fix moved. Behind it sits a second, latent reason grading never reaches today: the item also requires zero `erc.missing_tie` from a tie the run actually checked, and no `ties[]` is declared at all (disclosed as `tool_limitation`, klayout-tools#2169) — the state `supply_spec_disclosed_tool_limitation` names. Also note "one island" is not "reaches a pad" — a distinction this row used to carry as an open gap and no longer does: analog `GND` gained a drawn top-level pin on 2026-09-24 (**#362**, `spec/decision-records/DR-012-analog-ground-pad.md`), and a drawn mesh joining all three analog sub-blocks' own ground terminals to it later the same day (**#377**, `spec/decision-records/DR-013-analog-ground-mesh.md`); the ERC number is unchanged by either, which is exactly why both gaps had to be closed in the layout rather than in the report — and why #377's record carries an ERC ablation (remove the mesh and `GND` splits into 2 islands) rather than pointing at the clean run. The `signoff/t1-report.json` rows are graded from the envelope, not from this sentence |
