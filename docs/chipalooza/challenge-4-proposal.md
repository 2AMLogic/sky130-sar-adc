...
the current `reports/LATEST` — at an identical DRC/LVS verdict, identical
device/net/pin counts and identical mismatch categories; `compose.json`'s
top-level `bbox_um` was in turn byte-identical to `20260915-213439-bf2256f`'s,
verified directly, and moved by 0.05 µm in x0 only at the #326 re-composition,
where the external `VDD` pin's own met4 landing pad widened), which §4's Area
...
document, **11** are attached and therefore checked; of the **14** skipped,
**8** name a record stamp within 200 characters after the phrase, and **6**
name none at all.
...
1. **Top-level layout exists but is neither LVS-clean nor post-layout
   ...
   **Update this pass (2026-09-16): the 124-mismatch regression named above
   ...
   [`layout/sar-adc-top/reports/20260918-191315-935ce76/record.md`](../../layout/sar-adc-top/reports/20260918-191315-935ce76/record.md),
   the current `reports/LATEST`, at a field-identical DRC/LVS verdict;
   `compose.json`'s top-level `bbox_um` was byte-identical to
   `20260915-213439-bf2256f`'s and moved by 0.05 µm in `x0` only at the #326
   re-composition — see §4's Area row). **#103 itself is `loom:blocked` again as of this pass**, per its
