...
1. **Top-level layout does not exist (the largest gap).** No assembled,
   ...
   **Residual, deliberately deferred (2026-09-15) — now resolved (this
   ...
   both rows are now re-pointed to `reports/LATEST`
   (`20260908-072857-80df05e`), with the same artefact-by-artefact
   ...
   **Update this pass (2026-09-15, later): the v0.5.0-built device-match
   ...
   #103's own PR #275 — which is open, not yet merged.** The build did not
   ...
   blocker chain is now two upstream issues deep rather than one. Because
   PR #275 has not merged, its `layout/sar-adc-top/reports/20260915-213439-
   bf2256f/` build is not yet part of this repository's own committed
   evidence tree — §4's two sign-off-bar rows note the finding without
   re-pointing their citation path to it, and this item will cite the record
   by path once #103 lands it.
...
8. **Whole-ADC (end-to-end) code correctness is not yet demonstrated — a
   ...
   **Two root causes are now root-caused, evidenced, and confirmed still
   ...
   schematic** — the campaign's `reports/LATEST` record
   (`20260912-002315-9aaf1ca.md`) still predates PR #270's merge and still
   reports 0/9 corners code-correct at the near-full-scale inputs it does
   not yet cover the fix for. The **Sample rate** row in §4 is updated this
   pass to reflect this campaign's existence and status rather than
