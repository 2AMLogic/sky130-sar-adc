...
- **`klt erc` (power delivery, structural)**: **all four declared supplies
  ...
  **layout** (the supply spec's *graded* fields — `stackup`, `vias`, `nets[]`,
  ...
  ([`erc-reports/20260926-081822-203cca3/record.md`](../../layout/sar-adc-top/erc-reports/20260926-081822-203cca3/record.md),
  the first record in this chain graded on *different geometry* rather than on
  re-minted prose — issue #440 placed DR-017's two per-domain decoupling
  capacitors, so each of the four declared supplies gained drawn conductor —
  which supersedes `20260925-044420-f039594` (issue #364's third prose re-mint),
  ...
  this bullet — and neither did #440)
...
> of the **33** (spec row, `sim/` record) citation pairs in Section 4's
> table, **15** name a record that declares the full **9**-point grid,
> **13** name one that declares a smaller PVT point set, and **5** name one
...
> PVT point set), `sim/mc-smoke/records/20260814-021002-98d9186.md` (**1**
> point),
...
> point set),
> `sim/sampling-frontend/records/20260821-072657-433a294.md` (no PVT point
> set), `sim/sampling-frontend/records/20260825-021113-a237c79.md` (**1**
> point),
...
**This census also got worse-looking on 2026-09-26, for the same reason the
one below it did.** Nine pairs joined it (24 → 33) because §4 started citing
five records it had not cited before — four benches under the **Architecture**
row that `sim/spec-coverage.json` indexes there and the document cited none of
(see that row, and §7 Item 2, for how check 11's `structural`-row exemption
hid it), plus the two smoke-test records the **Corners** row had named only by
directory. Five of the nine land in the exception list, four of them at one
point or none: `sim/sampling-frontend`'s and `sim/sampling-cdac-handoff`'s
records are `tt`-centric sub-block diagnostics that never claimed a grid, and
`sim/mc-smoke`'s is a one-point harness control. **Citing them lowers this
census's headline percentage and raises the document's honesty**, which is the
trade this gate exists to keep visible rather than to optimise.
...
|---|---|---|---|---|
| Architecture | charge-redistribution SAR, differential, top-plate sampling | DRAFT (descriptive) | **MET** — implemented as described: charge-redistribution SAR (`design/cdac/cdac_array.sch`), differential throughout (no single-ended mode exists, §6, [DR-005](../../spec/decision-records/DR-005-cdac-array-design.md)), top-plate sampling (`design/sampling_frontend.sch`). Graded against the descriptive-row form of MET defined above — a topology claim against the schematics, not a bounded number at a corner. **Update this pass (2026-09-26): this row now also cites the four benches [`sim/spec-coverage.json`](../../sim/spec-coverage.json) indexes under it, which it had never cited.** The index is this repo's stated source of truth for "is this spec row addressed by a runnable testbench, and where is its evidence?", and check 11 enforces exactly that — no row may be graded while ignoring a campaign indexed under it — but check 11 skips any row whose `claim_class` is outside `ratified-measured`/`draft-informational`, and this row's is `structural`. So four benches sat under a row that cited none of them, and two of the four (`sim/sampling-frontend/`, `sim/sampling-cdac-handoff/`) are indexed nowhere else and were therefore cited nowhere in this document at all. **No part of this row's MET verdict moves**: it is still a topology claim against the schematics, and these records are what exercised that topology block by block rather than a numeric grade of it. The omission was load-bearing elsewhere, though — see the Sample rate row's own 2026-09-26 update and §7 Item 2 — and the general case is now gated by check 37 | `design/sar_adc_top.sch`, `design/cdac/cdac_array.sch`, `design/sampling_frontend.sch`, `design/comparator.sch`, `design/sar_sequencer.sch`; and the four benches the coverage index puts under this row: [`sim/sampling-frontend/records/20260821-072657-433a294.md`](../../sim/sampling-frontend/records/20260821-072657-433a294.md) (top-plate sampling: in-sample settling and the post-edge delta on `TOP_P`/`TOP_N`, issue #52) and [`sim/sampling-frontend/records/20260825-021113-a237c79.md`](../../sim/sampling-frontend/records/20260825-021113-a237c79.md) (the same bench's hold-kick diagnostic, issue #61 — its Experiment 6 is one of only two runs in this tree that measure the front end's own acquisition/hold residual on `TOP_x` with the real CDAC array attached; whole-ADC campaigns drive that assembled load too, but measure other quantities — see the Sample rate row); [`sim/sampling-cdac-handoff/records/20260824-231304-144edeb.md`](../../sim/sampling-cdac-handoff/records/20260824-231304-144edeb.md) (the front-end/array bottom-plate interface as `design/sar_adc_top.sch` actually wires it, issue #95 — the other one); [`sim/cdac-array-transfer/records/20260821-062504-433a294.md`](../../sim/cdac-array-transfer/records/20260821-062504-433a294.md) (charge redistribution: the array's own code-to-output transfer characteristic, issue #53); [`sim/sar-sequencer-behavioral/records/20260912-004028-9aaf1ca.md`](../../sim/sar-sequencer-behavioral/records/20260912-004028-9aaf1ca.md) (successive approximation: MSB-first bit-by-bit search and phase sequencing, issue #55 — this flow's current `records/LATEST`; the coverage index still names this bench's original 2026-08-23 record under this row, which is legitimate for a `structural` row whose claim is topology rather than a number, but this document cites the current one so check 3 grades it) |
...
> of the **30** (spec row, evidence flow) citation pairs in Section 4's
> table, **18** name a flow that publishes a `LATEST` pointer and are
> therefore freshness-checked by check 3; the remaining **12** name a flow that
> publishes none, whose current record nothing grades:
> `sim/cdac-array-transfer` (**4** records), `sim/comparator-decision`
> (**14** records), `sim/harness-corner-smoke` (**1** record),
> `sim/mc-smoke` (**1** record), `sim/sampling-cdac-handoff` (**1** record),
> `sim/sampling-frontend` (**2** records).
...
**The list grew from two flows to six on 2026-09-26, and that is the gate
working rather than a regression.** The four that joined —
`sim/harness-corner-smoke` and `sim/mc-smoke` (1 record each),
`sim/sampling-cdac-handoff` (1) and `sim/sampling-frontend` (2) — are
campaigns §4 previously either cited by bare `records/` directory (the
**Corners** row) or did not cite at all (the **Architecture** row); check 37,
added the same pass, is what surfaced the second case. Citing them by path is
strictly more honest than not citing them, and the price is that four
pointerless flows now appear here where nothing appeared before: the coverage
this census reports got *worse-looking* because the document got more
specific, which is exactly the direction this gate is meant to make visible.
Three of the four hold exactly one record, so the "hole" each opens is the
narrowest kind — there is only one record a citation of that flow could have
meant.
...
**A third census, one level further out (check 37, added 2026-09-26).** The two
above grade *cited* records — whether a citation is current, and what grid the
record it names declares. Neither can see a campaign this document never cites
at all, and nor can anything else here: checks 3/4/5/23 grade citations that
exist, check 6 counts pointer claims, check 11 grades a row against
[`sim/spec-coverage.json`](../../sim/spec-coverage.json) but skips any row whose
`claim_class` is outside `ratified-measured`/`draft-informational`. So the
index's one `structural` row (**Architecture**) carried four benches that row
cited none of, two of them indexed nowhere else and therefore cited **nowhere in
this document**, with no check able to report it. That omission was not
cosmetic — it is how the Sample rate row came to say "all four constituent
mechanisms … PVT-complete" without stating that mechanism (d)'s grid was run at
half the assembled top-plate load, because the only two campaigns that carry the
full load were the two that went uncited (§7 Item 2 has the full trace). The
general case is now gated:

> of the **14** campaigns under `sim/` holding at least one committed record,
> **14** are cited by path in this document and **0** are not: **none** — every
> campaign holding a committed record is cited by path

`--stats` prints that sentence live too. "Cited by path" deliberately means a
record path, not a mention of the campaign directory: this document names
`sim/vcm-drive-budget/` and `sim/full-conversion-transient/` in prose in a dozen
places each, and saying a campaign exists is not saying which of its records a
claim rests on — a gate that accepted the directory mention would have passed
the document this check was written against, which names
`sim/sampling-frontend/`'s testbench path repeatedly while citing none of its
records. An uncited campaign is not forbidden; it must be **named** here with
its record count, so the size of the hole is stated rather than absent.
...
examples below included): of the **29** "current `…/LATEST`" phrases in this
document, **16** are attached and therefore checked; of the **13** skipped,
**7** name a record stamp within 200 characters after the phrase, and **6**
name none at all.
...
1. **Top-level layout exists but is neither LVS-clean nor post-layout
   ...
   **Update (2026-09-24): the release gate cleared, the LVS mismatch did
   ...
   - The `--abstract-cells` collapse this item's earlier updates measured
     ...
     [`layout/sar-adc-top/reports/20260926-081248-203cca3/record.md`](../../layout/sar-adc-top/reports/20260926-081248-203cca3/record.md),
     the current `reports/LATEST` (superseding `20260924-234053-66dca3c`, issue #377's analog
     ...
     substrate merge reaching a device, not a new mismatch category. None of
...
2. **Sample rate is not re-derived (narrowed this pass, not closed).**
   ...
   **Update this pass (2026-09-26) — "all four PVT-complete" is qualified,
   not withdrawn: mechanism (d)'s grid was run at HALF the assembled
   top-plate load, and the two campaigns that do measure this mechanism at
   the full load were cited nowhere in this document.** Nothing is re-measured here and no
   verdict moves; what moves is that a claim this document has repeated
   since 2026-09-08 now states its own scope.

   *What the PVT-complete record actually loads.*
   [`sim/sampling-acquisition-settling/records/20260908-051436-6ccd72d.md`](../../sim/sampling-acquisition-settling/records/20260908-051436-6ccd72d.md)
   reads `sim/sampling-frontend/testbench/sampling_frontend_dut.spice` **in
   place** rather than netlisting a combined DUT — its own *Netlist
   provenance* line says so, and `sim/spec-coverage.json` records the same
   reuse in that bench's `testbench_note`. So the node it measures carries
   only the front end's own `Csamp_p`/`Csamp_n`, ~4.43 pF/side per DR-004.
   `design/sar_adc_top.sch` ties `TOP_P`/`TOP_N` to the CDAC array as well,
   adding ~4.43 pF/side of bit capacitance: **roughly double**, and the
   binding-corner residual of an RC acquisition is not load-invariant.

   *The two campaigns that do measure it at that load, and what they
   report.* Exactly two runs in this tree measure this mechanism's own
   acquisition/hold residual on `TOP_x` at the combined load — other
   campaigns carry the same load while measuring other quantities, see the
   next paragraph — and neither was cited anywhere in this document
   before this pass:
   ...
   (issue #95 — 3 input points × 3 CDAC previous-code states at `tt`/27 °C/
   1.8 V, plus **one** directional `ss` point) and
   [`sim/sampling-frontend/records/20260825-021113-a237c79.md`](../../sim/sampling-frontend/records/20260825-021113-a237c79.md)'s
   Experiment 6 (issue #61 — `tt` only). The handoff record's single `ss`
   point reports **5.33 mV** single-ended residual on `TOP_P` at
   `worst_case_pp`, ~3.0× the provisional differential half-LSB
   (1.7578 mV), against the front-end-only grid's worst of 0.380 mV
   (~0.2×). That record does not bury it: it names the figure, attributes
   it to the doubled load in so many words ("roughly double the load
   DR-004's own `Sa`/`Sd` L=0.5 µm sizing fix was verified against"), and
   defers it to a future timing pass. **It is also pre-#236**, so it is not
   a live verdict either — #236's `Sa` re-gating and 16× `Cmsw` widening
   moved the front-end-only binding corner from 67.19 mV to 0.380 mV, and
   the combined case very plausibly clears too. Nothing in this tree
   measures whether it does.

   *What does carry the assembled load, and what it measures instead.* The
   scarcity above is about one measured quantity, not about assembled-load
   simulation: `design/sar_adc_top.spice` ties `xfe`'s `TOP_P`/`TOP_N` to
   `xcdac`'s, so EVERY campaign driving that netlist carries the combined
   ~8.9 pF/side pair — and both whole-ADC campaigns in this tree do.
   ...
   (issue #254 — complete conversions at `f_clk = 12 MHz` across the FULL
   ratified 9-point grid, post-#236; its own committed netlist snapshot
   `sim/full-conversion-transient/netlist-snapshots/20260912-002315-9aaf1ca.spice`
   carries the same `xfe` + `xcdac` pair on `TOP_P`/`TOP_N`) grades
   whole-ADC code correctness — **overall FAIL**, 0/9 corners resolve every
   input to its ideal code ±1 LSB, root-caused to the open
   architecture/sizing decisions #267 and #269 (Item 8 below) and not to
   acquisition — plus the per-rail power §4's Power row cites.
   [`sim/supply-impedance-sensitivity/records/20260925-204633-7339971.md`](../../sim/supply-impedance-sensitivity/records/20260925-204633-7339971.md)
   (issue #378 — the same DUT and the same stimulus across 3 supply-return
   networks at `tt`/27 °C/1.8 V; a later 2026-09-26 leg of the same flow
   takes five networks to the full 9-point grid) grades the code, phase and
   per-rail-current DELTAS a non-ideal supply return introduces. Neither
   measures the front end's own acquisition/hold residual on `TOP_x`, so
   "nothing in this tree measures whether it does" is said about that
   residual specifically, not about assembled-load simulation. It is also
   why #469 is not a re-run of either: what it adds is mechanism (d)'s own
   residual measured on the assembled node, which no campaign here reports.

   *Why no check saw the omission, and what now does.* `sim/spec-coverage.json`
   indexes both campaigns — under the **Architecture** row, whose
   `claim_class` is `structural`. Check 11 is the gate for exactly this ("no
   Section 4 row ignores a campaign indexed under it") but it skips any row
   outside `ratified-measured`/`draft-informational`, so that row's four
   benches went ungraded, and the two indexed nowhere else went uncited
   document-wide. Every other citation check grades citations that *exist*
   (checks 3/4/5/23), counts pointer claims (check 6), or censuses the grids
   of cited records (check 28) — an omission is invisible to all of them.
   Both halves are fixed: §4's Architecture row now cites all four benches,
   and **check 37** (added this pass) grades a document-wide census of which
   `sim/` campaigns holding committed records this document cites by path, so
   a campaign is either cited or named as uncited, never silently absent.

   *What is owed.* The measurement — the combined-load DUT re-netlisted from
   the current schematics and taken to the ratified 9-point grid — is a
   simulation task, not an evidence-compilation one, and is filed as **issue
   #469** rather than attempted here. This item stays open either way: it was
   open on the end-to-end figure before this update and still is.
...
9. **Power delivery is now structurally graded — the two digital rails failed
   ...
   **Issue #355 (2026-09-24) fixed it, in the layout and not in the gate.**
   ...
   [`layout/sar-adc-top/erc-reports/20260926-081822-203cca3/record.md`](../../layout/sar-adc-top/erc-reports/20260926-081822-203cca3/record.md)
   — the first in this chain graded on *moved geometry*, issue #440's placement
   of DR-017's two per-domain decoupling capacitors, each of which is new drawn
   conductor on an already-declared supply —
   ...
   **The "Islands, now" column above is hand-transcribed, and as of this pass
   ...
   > electrical islands; and it grades `20260926-081248-203cca3`, while
   > `reports/LATEST` there names `20260926-081248-203cca3`: **current**.
...
10. **The repository's own T1 evidence-tier scorecard is machine-graded, and
    ...
    **The readout is machine-checked** (check 17 of the [citation
    ...
    > `layout/sar-adc-top/erc-reports/LATEST` at **20260926-081822-203cca3**
    > against a pointer naming **20260926-081822-203cca3**,
    > `layout/sar-adc-top/reports/LATEST` at **20260926-081248-203cca3**
    > against a pointer naming **20260926-081248-203cca3**: **current**.
