...
- **`layout/sar-sequencer/`'s pin counts are asymmetric** — 30 layout / 28
  ...
  subcircuit. Named here rather than smoothed over. Whether that flow should be
  promoting those two as top-level pins at all is a question for the flow, not
  for this document, and is filed as issue #322.
- **The five records were not all produced under the same `klt` build.**
  `layout/comparator/` and `layout/sampling-frontend/` were re-run under
  `klt 0.5.0` (PR #275, the release `layout/requirements.txt` now pins);
  `layout/cdac-array/`, `layout/sar-sequencer/` and `layout/seln-inverters/`
  still sign off on `klt 0.4.0`-built records, as each record's own Provenance
  block states. That matters because the 0.4.0→0.5.0 bump demonstrably changed
  extraction behaviour on this design — the sampling front end's MIM-cap arrays
  grew 0.92 µm in y, and the capacitor-device-class regression
  (klayout-tools#1876) moved the *top-level* mismatch count from 98 to 124
  before PR #287 neutralised it (§7 Item 1) — so three of the five sub-block
  sign-off verdicts above have not been re-confirmed under the tool build the
  composed top level is now signed off with. No verdict is restated as
  doubtful on that basis here; the gap is filed as issue #323.
...
**9** name a record stamp within 200 characters after the phrase, and **5**
name none at all.
