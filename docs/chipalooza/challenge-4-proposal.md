...
8. **Whole-ADC (end-to-end) code correctness is not yet demonstrated — a
   ...
   **Two root causes are now root-caused, evidenced, and confirmed still
   ...
   still predates PR #270's merge (2026-09-12T02:07:51Z) and still reports
   0/9 corners code-correct at the near-full-scale inputs it does not yet
   cover the fix for. **Citation-freshness correction, this pass**:
   `sim/full-conversion-transient/records/LATEST` no longer resolves to
   that record — it now points to
   ...
   PR #270 merged — the evidence that informed the DR-009 fix, not a
   post-fix corner re-run. No full corner-grid record exists after PR #270
   at all, so the conclusion above is unchanged in substance: this item now
   cites the full corner-grid record directly by path rather than as
   `reports/LATEST`, since that pointer has since moved to a narrower,
   non-campaign record. The **Sample rate** row in §4 is updated this pass
