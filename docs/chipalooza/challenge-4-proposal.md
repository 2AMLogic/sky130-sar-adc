...
2. **Sample rate is not re-derived (narrowed this pass, not closed).**
   ...
   **Update this pass (2026-09-06): the sequencer's own logic delay was
   ...
   three of the four (comparator decision delay, sampling front-end
   acquisition, sequencer logic delay) now PVT-complete,
   `docs/characterization-report.md`'s
   sample-rate row and `sim/report/manifest.py`'s citations for it have
   been updated to cite all six records (four mechanisms, two of them with
   both a single-corner and a full-grid record) and to state the front-end
   finding honestly, alongside the other mechanisms' comfortable margins.
