#!/usr/bin/env python3
"""Render layout/top-glue/reports/<record-id>/record.md from the JSON
...
re-derive from exit codes" discipline, via the same shared
`_record_common.render_pnr_drc_lvs_record` helper
`layout/sar-sequencer/` and the superseded `layout/seln-inverters/` use.
...
def main() -> int:
    ...
    print(render_pnr_drc_lvs_record("Top-level glue bank layout record", args))
    return 0
