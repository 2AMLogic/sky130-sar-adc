# Layout DRC/LVS record: 20260825-135406-877505b
...
| LVS negative control: device parameter | mismatch | mismatch_count=34, category_counts={'device.body_unverified': 1, 'device.unmatched': 5, 'net.unmatched': 26, 'topology': 2} |
| LVS negative control: topology (shorted net) | mismatch | mismatch_count=32, category_counts={'device.body_unverified': 1, 'device.unmatched': 4, 'net.unmatched': 25, 'topology': 2} |
| DRC negative control: injected violation | violations | violation_count=1, rules=['diff.width.1'] |

The good-reference LVS run's `mismatch_count` (8) is nonzero while `status` is `"match"` -- all 8 entries are `severity: "warning"` (0 at `severity: "error"`; see `lvs.json`), and are documented, expected quirks of this minimal, fully symmetric cell:
...
- `topology`, ambiguous net pairing (x5): the array's unit devices are electrically interchangeable (no two devices share a net that would anchor a unique pairing), so `NetlistComparer` resolves the correspondence structurally rather than uniquely -- expected for a fully symmetric matched array, not a defect.
...
- Record ID: `20260825-135406-877505b`
- `klt` version: `klt 0.3.0` (pinned, see `layout/requirements.txt`)
...
- Repo state: `877505b6bb410428a877d779f3d53e78ebb9c026` on `feature/issue-104` (dirty)
