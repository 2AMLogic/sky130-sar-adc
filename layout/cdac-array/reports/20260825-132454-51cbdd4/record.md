...
- LVS vs. `design/cdac/cdac_array.sch`: **MATCH**
  - devices: layout=56 reference=56 matched=56
  - nets: layout=42 reference=42 matched=42

## Unit-element check
`klt lvs`'s `combine_devices` folds `w` parallel unit caps into one device before comparing, so an LVS match alone cannot tell a 1024-unit-element array from 18 single scaled plates. The *pre-combination* extracted device count can, and this record asserts it:
